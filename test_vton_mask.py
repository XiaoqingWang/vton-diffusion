...
if __name__ == "__main__":
    ...
    with model.ema_scope():
        for data in dataloader:
            ...
            shape = x.shape[1:]
            img_mask_org = data['img_mask_org'][:c.shape[0]]
            img_mask_org = img_mask_org[..., None]
            img_mask_org = img_mask_org.to(device)
            img_mask_org = rearrange(img_mask_org, 'b h w c -> b c h w')
            img_mask_org = img_mask_org.to(memory_format=torch.contiguous_format).float()
            
            init_image = data['image'][:c.shape[0]]
            init_image = init_image.to(device)
            init_image = rearrange(init_image, 'b h w c -> b c h w')
            init_image = init_image.to(memory_format=torch.contiguous_format).float()
            ...
            x_samples_ddim = (1-img_mask_org)*init_image+img_mask_org*x_samples_ddim
            ...
            for i in range(N):
                outpath = os.path.join(outdir, data['c_name'][i])
